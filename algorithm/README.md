...
Orientation, since this directory has served two different lines of work.
The current paper is `paper/niche_separation/note.tex`, on the AMBIDEXTROUS
problem; everything it needs is generated by the `rigorous/ambi_*.py`
scripts, each of which runs from a clean clone and regenerates the numbers
it is cited for. The `rigorous/gerver_*.py` and `rigorous/sigma_*.py`
scripts belong to an earlier second-variation line whose central inference
step was refuted; they are kept because `paper/PROGRAM.md` cites them by
name as the computational record behind the retractions, and they should
not be read as supporting any current claim.
