...
| 74 | **UPPER BOUND: $A_{\text{ambi}}\le2\sqrt2-1=1.8284271\ldots$ in closed form.** The set left when the two quadrants are removed from the band splits into exactly four pieces; two are translates of one profile integral $g$ and two are cross terms that vanish unless a corner offset is positive. $\max g=g(1/\sqrt2)=2\sqrt2-1$, so $\lvert\mathcal C_2\cap L_{\pi/4}(c)\cap R_{\pi/4}(d)\rvert\le2\sqrt2-1$ whenever both corner offsets are nonpositive, as for $\Sigma$; a four-case analysis of the offsets (wide, both-positive, mixed, plus one certified two-variable profile inequality) then removes the hypothesis entirely: **$A_{\text{ambi}}\le2\sqrt2-1$ unconditionally.** **Sharp** — attained at $a=a\'=b=b\'=\sqrt2/4$. Brackets the answer: $1.6449552\le A_{\text{ambi}}\le1.8284271$, a factor of $1.1115$. **The first bound specific to the ambidextrous problem**; the previous one, $2.2195$, was inherited from the one-corner theorem. Replaces a withdrawn branch-and-bound claim of $1.86$ whose a priori box argument was invalid | PROVED (hand + one 28512-box ball certificate with negative control) |
...
| 74 | **UPPER BOUND: $A_{\text{ambi}}\le1.86$.** Branch and bound over the three-parameter reduction, $1\,546\,327$ boxes, all discharged in $688$s, behind a soundness gate that tests the box bound against the true objective on $36\,000$ points before any target is attempted. With Romik's sofa this brackets the answer: $1.6449552\le A_{\text{ambi}}\le1.86$, a factor of $1.131$. **The first bound specific to the ambidextrous problem**; the previous one, $2.2195$, was inherited from the one-corner theorem | PROVED (computer-assisted) |
...
algorithm/rigorous/ambi_* computations for the paper; every number is regenerated
                          ...
                          ambi_closedform.py  the closed-form bound 2 sqrt2 - 1 (Rule 3 checks)
                          ambi_cross.py       the cross region: unconditional theorem
                          ambi_certbound.py   branch and bound (superseded, kept for the gate)
...
Lean 4.33. **854 theorems, no `sorry`.** The 114-theorem core (`MovingSofa/Basic.lean`)
...
so the identities stay decidable. The remaining 740 theorems use Mathlib. Instantiating
