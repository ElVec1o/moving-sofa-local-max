...
\begin{equation}\label{eq:F1-computed}
   K=16:\ \text{negative definite},\ m_{1}=0.279;
   \qquad
   K=24:\ \text{negative definite},\ m_{1}=0.109,
\end{equation}
computed at $25$-digit working precision from proved closed-form
integrands.  Every computed eigenvalue is strictly negative --- this is
(F1) --- but the margin $m_{1}(K)$ \emph{decreases} with the truncation,
reflecting the constant-level competition just described; whether it
stabilizes at a positive value or decays to $0$ is precisely the
uniform-rate question of (F2), and we do not prejudge it.
...
outcome of the $m_{1}(K)$ trend.  Pointwise negativity is supported on
all fronts: the interior G\aa rding inequality (spread directions), the
blow-up profiles and $\chi$-forms (concentrated directions), and every
computed block eigenvalue.

What remains for (F2) is sharper than a generic cross-term estimate:
the junction-chord terms pair a slope jet at one breakpoint with a
value jet at another, giving a bilinear bounded by
$C\|\eta\|_{H^{1}}\|\eta\|_{H^{2}}$-interpolation with an explicit
geometric constant; by the interpolation identity
$\|\eta'\|^{2}\leq\|\eta\|\|\eta''\|$ this competes at
\emph{constant level} (not scaling level) with the coercive term, so
(F2) reduces to one explicit constant comparison plus the far-tail
decay --- finite, sharp, and honest.  The blow-up measurements of
Section~\ref{sec:blowup} already exhibit the competition resolving
favourably ($|J|\leq10\%$ of the coercive term on the worst localized
directions).
