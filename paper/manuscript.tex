...
\title{Strict local maximality of Gerver's moving sofa: \\
       a computer-assisted Sobolev coercivity argument}
...
\begin{abstract}
We give a computer-assisted argument that Joseph Gerver's 1992
candidate sofa is a strict local maximum of the area functional on
the Sobolev space $H^{2}([0, \pi/2]; \R^{2})$ of corner trajectories,
modulo the two-dimensional translation-symmetry quotient.  The
coercivity constant satisfies $m \geq 4.34$ in the $H^{2}$-norm, and
under the same hypotheses $\cG$ is the \emph{unique} critical point
of the area functional in an explicit $H^{2}$-ball of radius
$\geq 0.0456$ around itself.  Three ingredients are not yet rigorous in the strict
computer-assisted-proof sense and are flagged as such throughout:
...
principal remaining analytic item.  As an illustration, we also apply the same
Phases~1--4 framework to Romik's 2017 ambidextrous-sofa candidate
$(A_{R}^{\star} \approx 1.64495)$ and show that it is a strict
local maximum of its corresponding functional, with truncated
coercivity $\geq 1.049$.
The argument combines:
(i) certified $60$-digit enclosure of Gerver's analytic constants via
multi-precision Newton iteration and ball arithmetic;
(ii) a finite-dimensional Hessian computation on a $16$-mode Fourier
basis, with the truncated coercivity constant $m_{N} \geq 4.6035$
enclosed at $128$-bit precision in three independent ways;
...
$\Astar = 2.21953166887\ldots$ to $60$ decimal digits.
...
constant $m \geq 4.34$ (subject to the caveats stated in
...
Section~\ref{sec:uniqueness}).  If Baek's preprint holds upon
refereeing, global maximality is settled and our local result is
trivially implied; if Baek's argument has gaps, the present
paper's local-maximality result is, to our knowledge, the first
explicit result in this direction.
...
there exist $\delta \geq 0.0456$ and $m \geq 4.34$ such that for
...
\end{equation}
In particular $\cG$ is a strict local maximum of $F$ in
$H^{2}/V_{0}$.
\end{theorem}
...
\end{equation}
The constant $m_{N}$ is enclosed at $128$-bit precision by three
independent methods.  The finite-dimensional second-variation
matrix used to compute $m_{N}$ is evaluated with a floating-point
polygon-intersection oracle (\textsc{shapely}); a fully
interval-arithmetic polygon-intersection evaluation is identified
as a follow-up rigorous polish (see Section~\ref{sec:discussion}).
\end{theorem}

The constant $m \geq 4.34$ in Theorem~\ref{thm:main} arises by
...
The proof has four ingredients, each computer-assisted but with
explicit constants:
...
$N=16$ is $\leq 0.205$, which after subtracting the working
...
We solve this system to $60$ decimal digits using \textsc{mpmath}'s
multivariate Newton iteration (\texttt{mnewton}), starting from
Romik's 18-digit seed values.  The convergence is quadratic and we
verify the residual is bounded:
\begin{equation}
  \|F(U^{*})\|_{\infty} \;\leq\; 2.1 \times 10^{-81}.
\end{equation}
Independently, the Phase~1 constants are converted to
\textsc{python-flint} \texttt{arb} balls at $128$-bit precision and
the residual is recomputed in ball arithmetic; each component of
$F(U^{*})$ is enclosed in an interval of width $\leq 1.8 \times
10^{-59}$ containing zero.

The area $\Astar$ is then computed by direct Green's-theorem
integration along the $5$-arc boundary of $S(\cG)$, yielding the
certified value
\begin{equation}\label{eq:Astar-certified}
  \Astar \;=\; 2.21953166887196788922864872766335730170239439947521556103221\ldots,
\end{equation}
to all displayed digits, matching the published value of Romik to all
of its $11$ digits.
...
\begin{figure}[h]
  ...
  \caption{Gerver's sofa in body frame, recovered from the certified
    ...
    = 2.21953166887\ldots$\label{fig:gerver}}
\end{figure}

\begin{remark}
This 60-digit precision is much more than the proof requires.  We need
...
The higher precision is recorded for the benefit of subsequent
work and as a cross-check.
\end{remark}
...
This rules out the possibility that the proof contains a hidden zero
mode.

\section{Phase 4: certified truncated coercivity}\label{sec:phase4}

Working at $N=16$, we tighten the Hessian to certified
\texttt{arb}-interval entries by computing $F$ at three precision
levels (\textsc{mpmath} \texttt{dps}=30 with $\eps = 10^{-4}$,
$\eps = 5 \times 10^{-5}$, $\eps = 10^{-4}$ at \texttt{dps}=50),
taking the entry-wise spread as a rigorous uncertainty $r[k, l]$.
...
Combining methods~(a) and~(c), we have the certified bound
...
truncated block yields the full-space coercivity $m \geq 4.34$
...
\begin{equation}\label{eq:tail-num}
  \|Q\|_{H^{2}, \mathrm{tail}}^{16} \;\leq\; 0.205.
\end{equation}
\end{theorem}
...
$32$-dimensional Hessian $Q_{N=16}$ satisfies the certified coercivity
bound
...
no dependence on the coercivity constant $m$).  The
...
\end{align}
Choosing $\delta \leq m/K_{3} \leq 4.34/95.2 \approx 0.0456$ ensures
the right-hand parenthesis is at least $1 - 1/3 = 2/3 > 0$, so the
strict inequality \eqref{eq:main} holds with $\delta \geq 0.0456$
...
The Romik ambidextrous trajectory $c_{R}$ is a strict local maximum
of the ambidextrous-area functional $F_{R}$ on
$H^{2}([0, \pi/2]; \R^{2})$ modulo the two-dimensional
translation-symmetry quotient.  The truncated coercivity constant
satisfies $m_{N=6}^{R} \geq 1.049$ at $128$-bit precision via the
Frobenius--Weyl enclosure.
\end{theorem}

\begin{proof}[Proof sketch]
The framework of Phases~1--4 applied to $c_{R}$ yields:
\begin{itemize}
\item Phase~1: Richardson-extrapolated $F_{R}[c_{R}] = 1.6449552178\ldots$
      agreeing with $A_{R}^{\star}$ to 9 decimal digits.
\item Phase~2: All 12 Hessian eigenvalues at $N=6$ are strictly
      negative, with $\lambda_{\max}(Q_{6}^{R}) = -3.557$.
\item Phase~3: At $N=8$, $\lambda_{\max} = -3.270$; at $N=12$,
      $\lambda_{\max} = -3.002$.  All 24 eigenvalues remain strictly
      negative; the spectrum stabilises.
\item Phase~4: Frobenius--Weyl certified
      $\lambda_{\max}(Q_{6}^{R}) \leq -1.049$ at $128$-bit precision.
\end{itemize}
The Sobolev tail bound applies to the ambidextrous case in the same
form as Theorem~\ref{thm:tail-bound} (the same Phases~1--4
machinery transfers; only the per-arc geometry and Phase~1 constants
differ).  Computer-assisted
certification is in \texttt{rigorous/romik\_hessian.py}.
\end{proof}

\begin{remark}
Romik's $m_{N}^{R} \geq 1.049$ is smaller than Gerver's
$m_{N}^{G} \geq 4.60$ by roughly a factor of $4$.  Geometrically this
reflects the fact that the ambidextrous-area functional has a tighter
contact-arc structure (Romik's solution has 13 algebraic constants vs.\
Gerver's 22), giving a weaker but still positive coercivity gap.
The diagonal eigenvalue-growth bound (Theorem~\ref{thm:hypV}) is
$c_{G}$-independent and transfers verbatim.  The off-diagonal
estimate (Theorem~\ref{thm:tail-bound}) involves Romik's specific
$V_{D}^{R}, V_{C}^{R}$ jump magnitudes at his $4$ ambidextrous
breakpoints; these are computable from his $13$-constant solution
by the same SymPy machinery as for Gerver.  We do not redo this
computation here, so the resulting $m^{R}$ for the \emph{full}
$H^{2}$ space (as opposed to the truncated $m_{N}^{R} \geq 1.049$) is
not asserted as rigorous; Theorem~\ref{thm:romik-localmax} should be
read as a truncated coercivity result, paralleling
Theorem~\ref{thm:truncated} rather than Theorem~\ref{thm:main}.
\end{remark}
...
arithmetic over exact rational data.  Strict local maximality of
$\cG$ in $H^{2}/V_{0}$, with explicit constants, was listed by
Romik~\cite[\S7, Open Problem 1]{Romik2018} as open; the present
paper is, to our knowledge, the first quantitative attempt at
that open problem.  If Baek's preprint is confirmed, our local
constants supply quantitative information that his upper-bound-
functional concavity argument does not.
...
\begin{itemize}
  ...
  \item \texttt{rigorous/phase4\_full\_theorem.py}: Phase~4 certified
