...
$C/|k-l|$ structure of \eqref{eq:entry-asymp}; the remainder beyond
$L=192$ is governed by the same oscillatory bands at gaps $>128$
(measured $\leq0.006$ and falling), whose Abel-summed operator norm
is the one remaining half-page of classical analysis --- its
crude size ($\lesssim0.05$) leaves $\tau \leq 0.52 < 0.62$ with
room.
Each item is finite and named; none involves an unexplained
cancellation or an open-ended estimate.  We believe this is an
accurate statement of exactly how far the second-variation route to
local maximality can presently be carried, and of what remains.
...
radius $10^{-10}$ about the $25$-digit assembled values; replacing
that premise by interval quadrature of the same closed-form
integrands is the one remaining mechanical step of (F3).
