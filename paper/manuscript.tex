...
\begin{remark}[Status of the criterion]\label{rem:quartic-status}
We do not prove \eqref{eq:quartic-crit} holds uniformly in the
perturbation direction, and we caution that the margin is delicate.
Under the concentrating normalization $\|\eta\|_{H^{2}}=1$, the
perturbations that stress \eqref{eq:quartic-crit} change the area below
the floating-point resolution of the Shapely oracle (measured
$|\delta F|\approx 2\times10^{-7}$ at $w=0.005$, at the noise floor).
Deciding the criterion in the worst directions therefore requires the
interval-arithmetic oracle of Section~\ref{sec:discussion}, caveat~(i).
What we assert is structural and, we believe, the right reformulation:
local maximality of $\cG$ is equivalent to a finite family of quartic
inequalities \eqref{eq:quartic-crit} at the four breakpoints, not to a
coercivity estimate in any single Sobolev norm.  This is why the
problem resists the standard second-variation machinery, and it is the
sense in which the present analysis reduces the open question rather
than resolving it.
\end{remark}
