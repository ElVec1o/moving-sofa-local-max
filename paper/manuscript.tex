...
quotient.  These are not theorems in the strict
computer-assisted-proof sense; they
are conditional on three explicitly identified non-rigorous
ingredients~(i)--(iii), flagged throughout.  Conditional on these,
...
Section~\ref{sec:uniqueness}).  By Baek's resolution, global
maximality of Gerver's sofa implies its local maximality, so the
present paper is not in doubt
...
\emph{mode-uniform} ($\approx -5.9$ across $2 \leq k \leq 16$) rather
than oscillatory, and a fit at the breakpoint frequencies is not
statistically distinguishable from a fit at random frequencies, nor
from a smooth $a + b/k$ model.  We read this as evidence that the
dominant part of $Q_{\mathrm{jump}}$ is \emph{not} finite-rank
point support but rather an effective differential-operator
contribution (an additional multiple of $\|\eta'\|^{2}$), with the
genuine breakpoint-jet terms present but subdominant.  This is a
negative result for the cleanest structural hypothesis, but it
sharpens the target: a closed-form $Q_{\mathrm{jump}}$ must reproduce
the mode-uniform $k^{2}$ growth, not merely the four point
contributions.  The diagonal alone cannot separate the two; a
decisive test requires the full off-diagonal operator together with
an assembled $Q_{\mathrm{smooth}}$.
...
mode-uniform $k^{2}$ rather than oscillating at the breakpoint
frequencies, so $Q_{\mathrm{jump}}$ is not the finite-rank
point-supported operator one might first guess but carries a
dominant effective $\|\eta'\|^{2}$ contribution that any closed
form must reproduce.
\end{enumerate}
