...
Resolving the criterion requires $C_{4}$, whose signature in the area
is $\sim10^{-6}$ --- only a few times the Shapely floating-point floor
$\sim2\times10^{-7}$, so the polygon oracle cannot measure it.  We
therefore evaluate $F$ instead by the exact Green's-theorem boundary
integral over the analytic contact paths
(\texttt{rigorous/analytic\_oracle.py}), in \textsc{mpmath} at $30$
digits.  This oracle reproduces $A^{\star}$ to
$4\times10^{-16}$ (against Shapely's $2\times10^{-7}$), so
$Q,C_{3},C_{4}$ become clean; it is also the interval-ready form
identified in Section~\ref{sec:discussion}, caveat~(i).  At the $b_{2}$
breakpoint (\texttt{rigorous/phase2o\_criterion\_exact.py}) it gives,
in the $x$-component,
\[
  \begin{array}{lcccc}
    w:      & 0.020 & 0.015 & 0.012 & 0.006\\
    C_{4}:  & +16.7 & -6.5  & -16.9 & -26.6\\
    \text{margin of \eqref{eq:quartic-crit}}: & (C_{4}>0) & -62\% & +42\% & +27\%
  \end{array}
\]
So $C_{4}$ changes sign near $w\approx0.015$, and away from that
crossover, in the $C_{4}<0$ regime, the criterion holds with a
comfortable margin ($+24\%$ to $+42\%$ across the $x$- and
$y$-components) --- not the marginal figure a floating-point evaluation
suggests.

\begin{remark}[The crossover, and what the oracle does and does not settle]
\label{rem:quartic-status}
At the crossover width where $C_{4}\approx0$ the quartic model
\eqref{eq:ray-poly} degenerates and \eqref{eq:quartic-crit} is not the
right test: there the higher-order coefficients $C_{5},C_{6},\dots$
govern.  This is not a threat to local maximality.  Evaluating $g$
directly with the analytic oracle at the crossover width $w=0.015$, we
find $g(\varepsilon)<0$ at \emph{every} tested $\varepsilon$ out to
$\|\varepsilon\eta\|_{H^{2}}\approx4$ --- including $\varepsilon=0.224$,
exactly where the truncated quartic predicts $g>0$
($g(0.224)=-3.7\times10^{-4}$).  So the ``failure'' of
\eqref{eq:quartic-crit} at the crossover is a failure of the quartic
\emph{truncation}, not of concavity: the full $g$ stays negative to all
orders.  Across all probed widths at $b_{2}$, then, $\cG$ is a local
maximum along the ray, and in the genuinely narrow regime it is so with
the comfortable $+24\%$ to $+42\%$ margin above.

What remains open is a \emph{proof}.  We have verified the criterion at
high precision at the $b_{2}$ breakpoint over a range of localized
directions, not established it uniformly in the direction $\eta$, and
not for all four breakpoints.  A proof requires a closed form for
$C_{3}(\eta),C_{4}(\eta)$ --- obtained by carrying the second-order
implicit-function response \eqref{eq:c34} through the contact-transition
conditions of Section~\ref{sec:jump-piece} --- together with the
higher-order treatment at the crossover.  The reformulation we assert is
structural: local maximality of $\cG$ is controlled by a finite family
of scale-resolved conditions at the four breakpoints, not by a
coercivity estimate in any single Sobolev norm.  The healthy margins
above, now free of the floating-point floor, indicate the target is
comfortably true rather than marginal.  For $\cG$ the question is in any
case settled by Baek~\cite{Baek2024}; the value of the reduction, and of
the exact oracle, is that both apply where no global result exists ---
once the prior symbol degeneracy (Remark~\ref{rem:sigma-harder}) is
overcome.
\end{remark}
