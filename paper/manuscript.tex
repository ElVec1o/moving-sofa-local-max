...
This is how a proof should go, and it isolates exactly the one missing
estimate.  Writing $g(\varepsilon\hat\eta) = \tfrac12 Q(\hat\eta)
\varepsilon^{2} + R(\varepsilon)$, the local maximum follows once
$R(\varepsilon) = o(w^{2}\varepsilon^{2})$ uniformly on the ball --- i.e.\
once the moving-breakpoint remainder is controlled against the coercive
quadratic in the blow-up.  The blow-up profile \eqref{eq:blowup} makes
this a boundary-layer estimate at a single point rather than a global
Sobolev inequality, and its target is a \emph{negative} quantity with
room to spare ($-0.107$ against the bound $-0.100$).  We regard
establishing \eqref{eq:blowup} and this subdominance rigorously ---
together with the passage from the worst-case localized bumps to general
$\eta$, and \textsc{arb}-interval certification --- as the concrete
remaining path to a theorem.
