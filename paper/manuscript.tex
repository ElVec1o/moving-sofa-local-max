...
the jump across the moving limit,
\begin{equation}\label{eq:ray-poly}
   g(\varepsilon) := F[\cG+\varepsilon\eta]-F[\cG]
   = \tfrac12 Q\,\varepsilon^{2}
   + \tfrac16 C_{3}\,\varepsilon^{3}
   + \tfrac{1}{24} C_{4}\,\varepsilon^{4} + \cdots,
\end{equation}
where $Q = Q(\eta) < 0$ is the (coercive) quadratic form, and
\begin{equation}\label{eq:c34}
   C_{3} \;\propto\; b_{2}(a_{1}\beta + a_{2}) + \beta p_{2},
   \qquad
   C_{4} \;\propto\; b_{2}\bigl(\tfrac12 a_{1}b_{2} + p_{2}\bigr),
\end{equation}
derived symbolically in \texttt{rigorous/phase2m\_quartic\_criterion.py}.
Crucially the linear breakpoint response alone
($b_{2}=0$) produces \emph{no} cubic; the fatal cubic is generated by
the \emph{second-order} breakpoint response $b_{2}=\tfrac12 B''(0)$,
together with the quadratic part $p_{2}$ of the integrand.

\begin{proposition}[Quartic criterion for ray-concavity]
\label{prop:quartic}
If $C_{4}<0$, then $g(\varepsilon)\leq0$ for all $\varepsilon$ (in the
quartic model \eqref{eq:ray-poly}) if and only if
\begin{equation}\label{eq:quartic-crit}
   C_{3}^{2} \;\leq\; 3\,|Q|\,|C_{4}| .
\end{equation}
\end{proposition}

\begin{proof}
$g(\varepsilon)/\varepsilon^{2} = \tfrac12 Q + \tfrac16 C_{3}\varepsilon
+ \tfrac{1}{24}C_{4}\varepsilon^{2}$ is a downward parabola in
$\varepsilon$ when $C_{4}<0$; its maximum value is
$\tfrac12 Q + \tfrac16 C_{3}^{2}/|C_{4}|$, which is $\leq0$ exactly
when \eqref{eq:quartic-crit} holds.
\end{proof}

This converts the diffuse two-norm difficulty into a concrete finite
inequality: local maximality along the breakpoint directions holds iff
\eqref{eq:quartic-crit} holds at each $b_{j}$, in every direction.

The individual coefficients $C_{3},C_{4}$ are, however, numerically
delicate: they are third and fourth $\varepsilon$-derivatives of $g$,
and for a bump of width $w$ their finite-difference extraction carries a
truncation error that grows as $w\to0$ (the higher
$\varepsilon$-derivatives of $g$ scale with inverse powers of $w$).
Different step choices give different $C_{4}$, even of opposite sign, so
we do \emph{not} report the criterion through extracted $C_{3},C_{4}$.

What \emph{is} robust is $g$ itself, and local maximality is exactly the
...
floor that made the Shapely evaluation inconclusive.  The quartic model
\eqref{eq:ray-poly}--\eqref{eq:quartic-crit} remains valuable as
\emph{structure} --- it explains why the difficulty is concentrated in
the moving breakpoints and reduces the question to finitely many
conditions there --- but we do not rest any numerical conclusion on its
individually-extracted coefficients.
...
narrow-bump limit, which a closed form for the ray coefficients via the
second-order response \eqref{eq:c34} would make rigorous.  For
