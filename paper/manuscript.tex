...
order.  All four breakpoints live in the notch $N$, which we find
contributes only at second order in the concentration width.
...
remaining notch terms --- the $B$- and $D$-envelope arcs and the
breakpoint motion --- are what
Conjecture~\ref{conj:subdominance} asserts are $o(w^{2})$.
...
Every clause is verified to $30$ digits at all four breakpoints and both
components (Sections above); clause~(ii)'s leading coefficient is
\emph{proved} (it is the G\aa rding inequality
\eqref{eq:blowup-garding}).  What is open is (i)--(iii) as $w\to0$
\emph{statements}, i.e.\ the matched-asymptotic passage to the limit.
...
The obstruction to proving it directly is a cancellation we can exhibit
but not yet explain: the moving breakpoint \emph{a priori} contributes an
$O(w)$ term to $Q(\hat\eta_{w})$ --- through
$\Delta(\partial_{\theta}J)\,(\delta b)^{2}$ with a nonzero jump of the
boundary-integrand derivative (the sofa boundary has a genuine corner at
each breakpoint, Section~\ref{sec:blowup}) --- yet the oracle finds
$Q(\hat\eta_{w})=O(w^{2})$.  Some structural identity forces this
$O(w)$ term to cancel; identifying it is, we believe, the key to
Conjecture~\ref{conj:subdominance}.
