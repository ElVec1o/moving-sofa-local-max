...
computed windows --- the weld closes with a factor $\geq3.6$ of
margin wherever it can be checked, and the band structure of
\eqref{eq:entry-asymp} (entries $\lesssim C/|k-l|$ with oscillation)
is what keeps $\tau$ bounded as the window grows.  What remains to
make the $l>64$ coupling fully analytic is mechanical: compute the
cross block to a buffer $l\leq L$ with the Rust oracle (minutes at
$L\sim300$) and bound the remainder by the banded Hilbert--Schmidt
estimate, whose gap factor $1/\sqrt{L-64}$ makes it small.  With
$\tau_{\mathrm{far}}$ at its measured plateau $\approx0.45$, the
assembled coercivity constant is
\[
