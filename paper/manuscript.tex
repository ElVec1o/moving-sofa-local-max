...
This is itself informative --- it is the analytic fingerprint of the
moving breakpoint --- but it means the Taylor route stalls at third
order, and a decision about local maximality must be made from $g$
directly rather than from its coefficients.
...
genuinely different and we report it as such.  At accessible widths the
measured $G$ sits above the $\chi$-form by $J/(2w^{2}) \approx
+0.009$--$0.012$ (both components, both signs occur), i.e.\ the junction
correction $J$ is nonzero at finite $w$ and eats at most $10\%$ of the
coercive term.  Whether $J\to0$ as $w\to0$ (the $\chi$-form exact in the
limit) could not be decided: the junction parameter at $b_{2},b_{3}$
coincides with a phase boundary of $\cG''$, and the oracle's Newton
junction solver becomes unreliable there for $w\lesssim2.5\times10^{-3}$
(non-monotonic values, convergence failures --- documented in
\texttt{rigorous/phase2t\_bulk\_theorem.py}).  Resolving the limit
requires a kink-aware junction solver; until then the safe statement is
the finite-$w$ bound $|J|\leq0.1\times$ the coercive term over the
measured range, which suffices for the negativity of $G$ there.
