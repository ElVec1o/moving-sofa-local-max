...
Theorem~\ref{thm:no-h2}) but staying strictly negative.  So $\cG$ is a
local maximum along the localized directions at $b_{2}$, verified to $30$
digits --- and, unlike the $C_{3},C_{4}$ margins, this statement does not
depend on any finite-difference extraction.
...
verified $g<0$ at high precision at the $b_{2}$ breakpoint over a family
of localized directions and over an $H^{2}$-ball; we have \emph{not}
established it uniformly in the direction $\eta$, nor for all four
breakpoints, nor with certified (interval) arithmetic in place of $30$-
digit \textsc{mpmath}.  The natural route to close each of these is now
concrete: a closed form for the ray coefficients via the second-order
implicit-function response \eqref{eq:c34} through the contact-transition
conditions of Section~\ref{sec:jump-piece} (for uniformity in $\eta$);
the same computation at $b_{1},b_{3},b_{4}$ (the mirror symmetry
$\theta\mapsto\pi/2-\theta$ pairs $b_{1}\!\leftrightarrow\!b_{4}$,
$b_{2}\!\leftrightarrow\!b_{3}$, halving the work); and promotion of the
oracle from \textsc{mpmath} to \textsc{arb} intervals.  For $\cG$ the
statement is in any case a consequence of Baek~\cite{Baek2024}; the
value of the reduction and of the exact oracle is that both apply where
no global result exists, once the prior symbol degeneracy
(Remark~\ref{rem:sigma-harder}) is overcome.
\end{remark}
