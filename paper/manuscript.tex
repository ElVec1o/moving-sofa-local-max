...
\begin{lemma}[Far-tail bound]\label{lem:fartail}
Let $T_{\mathrm{far}}$ be the coupling of the low block ($N$ rows) to
tail modes at gap $\geq G$, with entries dominated by the oscillatory
band kernel $|a(m)| \leq c/m$ of \eqref{eq:entry-asymp}.  Then
$(T_{\mathrm{far}}T_{\mathrm{far}}^{*})_{kk'}$ has diagonal
$\leq c^{2}/(2(G-1))$ and off-diagonal carrying the coherent factor
$\cos(2(k-k')\theta_{R})$ --- a rank-two structure --- whence
\[
   \|T_{\mathrm{far}}\| \;\leq\; c\,\sqrt{\tfrac{N}{2G}}\,(1+o(1)) .
\]
With the measured band constant $c \leq 0.7$, $N = 124$, $G = 129$
this gives $\|T_{\mathrm{far}}\|\leq0.485$; the directly computed far
section (modes $193..280$, Rust oracle) measures
$\|T_{\mathrm{far}}\| = 0.1068$ --- the lemma is safe by a factor
$4.5$.  Extending, the total coupling saturates:
$\tau(65..280) = 0.4764$ against $\tau(65..192)=0.4689$, and the
remainder beyond $280$ obeys the same $\sqrt{1/G}$ scaling
($\leq 0.107\sqrt{129/217} = 0.082$), so
\[
...
augmentation of Section~\ref{sec:symmetry-quotient}.  What remains of
the assembly is one computed row --- the coupling of the four
slope-carriers to the slope-free tail, governed by the same
$1/l$-banded kernels --- and the three-block weld arithmetic; both
are of the kind already performed.
