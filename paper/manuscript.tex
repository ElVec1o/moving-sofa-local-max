...
$m_{1}$-trend $0.72,0.70,0.596$);
(iv) interval certification of the finitely many computed
quantities.
...
\paragraph{Certification protocol (F3).}
Every quantity entering (F1)--(F2) is a finite integral of an
explicit closed-form integrand (products of sines, cosines and the
phase masks) or a finite jet expression, assembled at $25$-digit
working precision; a doubled-precision consistency pass bounds the
assembly error far below the reported margins.  Full certification
consists of interval quadrature of the same integrands plus an
interval eigenvalue enclosure (Gershgorin or verified
diagonalization) of the resulting matrices --- entirely standard
machinery (\textsc{arb}/\textsc{flint}), not available in the present
computational environment and therefore recorded as the specified
final step rather than performed here.
