...
%
%  All constants are tracked.  At N = 16 the final bound is
%
%      ||Q||_{H^{2}, tail}^{16}  <=  0.0299  <  0.03
%
%  giving the coercivity margin
%
%      m  >=  m_{N} - 0.03  >=  4.6035 - 0.03  =  4.5735.
% =====================================================================
...
$\|Q\|_{H^{2},\mathrm{diag}}^{16} \leq 0.0042$ from
Theorem~\ref{thm:hypV} and the block-decomposition factor of $4/\pi$
already in \eqref{eq:final-numerical}, yields the consolidated
total
\begin{equation}\label{eq:total-tail}
   ...
   \;+\;
   \underbrace{0.200}_{\text{off-diagonal, four-block}}
   \;\leq\;
   0.205.
\end{equation}
...
possible but not required, since $0.205 \ll m_{N} = 4.6035$.
...
\leq 0.205$ and $m_{N} \geq 4.6035$ together yield $m \geq 4.398$.
\end{proof}

\subsection{Caveats and what is honest}\label{sec:offdiag-caveats}

\begin{enumerate}
\item \textbf{The bound $0.205$ is rigorous but conservative.}
The rigorous figure $0.205$ is obtained using the worst-case
...
sources contribute to this slack:
   ...
   stated rigorous figure $0.205$, because the numerical jump-sum
...
needed for the present theorem: $0.205 \ll 4.6035$.
...
constant $m \geq 4.398$; the cross-term placeholder
\eqref{eq:cross-term-working} in the main-paper proof further
reduces this to $m \geq 4.34$.  Both are comfortably bounded
