...
Second, connectedness does not by itself exclude it. For a single $t$, the pair
$Q_t\cup\rho Q_t$ removes a full vertical segment --- and so disconnects any body
meeting both sides of it --- exactly when the apex height exceeds a threshold
$h^\ast(t)$. Computation gives $h^\ast(t)>\tfrac12$ for every $t\in(0,\pi/2)$,
with $h^\ast(t)\to\tfrac12$ as $t\to0$ and $h^\ast(\pi/4)=0.5100$,
$h^\ast(1.52)=0.6967$. So no single-$t$ argument forces $M\le\tfrac12$, although
the pointwise constraint $c_y(t)\le h^\ast(t)$ does hold for every connected
ambidextrous sofa. Whether the full family forces $M<\tfrac12$ is open.

Consequently the reduction supports a conditional statement rather than an
unconditional one.

\begin{proposition}[Conditional form]\label{prop:cond}
Let $S$ be an ambidextrous moving sofa with rotation path $c$ satisfying
$\max_t c_y(t)<\tfrac12$. Then its two niches are disjoint and
